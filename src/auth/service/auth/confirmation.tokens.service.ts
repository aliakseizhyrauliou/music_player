...
export class ConfirmationTokensService {
  constructor(
    @InjectRepository(UserConfirmationTokenEntity)
  ...
  async getByUserId(userId: number): Promise<UserConfirmationTokenEntity> {
    return await this.userConfirmationTokenEntityRepository.findOneBy({
      user_id: userId,
    });
  }
  ...
  async GenerateToken(
  ...
  ): Promise<ConfirmationTokensServiceResponse> {
    ...
    const token: string = await this.generateRandomString(
      confirmationTokenLength,
    );

    const newTokenEntity: UserConfirmationTokenEntity =
      new UserConfirmationTokenEntity();
    newTokenEntity.token = token;
    newTokenEntity.user_id = dto.user_id;

    const resut =
      await this.userConfirmationTokenEntityRepository.save(newTokenEntity);

    return new ConfirmationTokensServiceResponse(
      true,
      '',
      resut.token,
      user.id,
  ...
  async VerificateToken(dto: VerificateDto): Promise<BaseResponse> {
    const tokenEntity: UserConfirmationTokenEntity =
      await this.userConfirmationTokenEntityRepository.findOneBy({
        user_id: dto.user_id,
        is_confirmed: false,
      });
    ...
    if (
      ...
      tokenEntity?.user_id !== dto.user_id
    ) {
